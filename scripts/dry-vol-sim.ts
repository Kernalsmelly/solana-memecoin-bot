...
if (process.env.FORCED_SEED_TOKENS && process.env.FORCED_SEED_TOKENS.includes('dummy')) {
  console.error(
    '[FATAL] FORCED_SEED_TOKENS is set to or includes "dummy":',
    process.env.FORCED_SEED_TOKENS,
  );
  process.exit(1);
}
...
import { ParameterSweepManager, SweepParams } from '../src/strategy/ParameterSweepManager.js';
...
async function main() {
  try {
    ...
    const liveMode = true;
    console.warn(
      '[WARNING] LIVE MODE ENABLED: This run will submit a REAL transaction to Solana mainnet!',
    );
    ...
    if (forceTrade && forceToken) {
      ...
      try {
        ...
        function isParsedAccountData(
          data: any,
        ): data is { parsed: { info: { decimals: number } } } {
          return (
            data &&
            typeof data === 'object' &&
            'parsed' in data &&
      ...
      debug(
        `[FORCED SIZE CONVERSION] Human: ${forceSize} | Decimals: ${decimals} | Base Units: ${forceSizeBaseUnits}`,
      );
    }
    ...
    if (forceTrade) {
      ...
      const outputMint =
        process.env.BASE_OUTPUT_MINT || 'So11111111111111111111111111111111111111112';
      debug(`[FORCED TRADE] inputMint (sell): ${inputMint}, outputMint (buy): ${outputMint}`);
      if (outputMint === 'dummy' || !outputMint || outputMint.length < 32) {
        console.error(
          '[ERROR] Refusing to trade with invalid/dummy mint at quote fetch:',
          outputMint,
        );
        process.exit(1);
      }
    ...
    coordinator.on('tokenDispatch', async (tokenAddress: string) => {
      ...
      try {
        ...
        if (inputMint === outputMint) {
          logger.warn(
            `[SwapLogic] Skipping swap: inputMint and outputMint are the same (${inputMint})`,
          );
        ...
        if (typeof handleDryRunFill === 'function') await handleDryRunFill(fill, riskManager);
        fills.push(fill);
        fillCount++;
        coordinator.completeToken(tokenAddress);
      } catch (e) {
      ...
      orderManager.on('orderFilled', (order) => {
        ...
        if (
          ...
          for (const params of sweepGrid) {
            ...
            const std = Math.sqrt(
              mockPnls.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / (mockPnls.length || 1),
            );
          ...
          logger.info(
            `[ParamSweep] Feedback sweep complete. Best params: ${JSON.stringify(bestParams)} (Sharpe-like: ${bestSharpe.toFixed(2)})`,
          );
      ...
      try {
        ...
        if (liveMode && fillCount >= maxTrades && !killSwitchEngaged) {
          await notificationManager.notify(
            'Test alert: Max trades reached in live-mode dry-run.',
            'all',
          );
          logger.warn('🚨 Live-Mode kill-switch engaged');
          // Flip kill switch
