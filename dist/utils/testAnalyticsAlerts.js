...
const getRecentTradeStats_1 = require("./getRecentTradeStats");
...
function writeTrades(trades) {
    ...
    fs_1.default.writeFileSync(TRADES_FILE, trades.map(t => JSON.stringify(t)).join('\n'));
}
...
async function runTestCase(name, trades) {
    writeTrades(trades);
    const stats = await (0, getRecentTradeStats_1.getRecentTradeStats)(config_1.analyticsConfig.analyticsWindowMinutes * 60 * 1000);
    ...
    let alerts = [];
    if (buckets.lt10 > 2) {
        alerts.push(`🚨 ALERT: ${buckets.lt10} trades with PnL worse than -10% in the last ${config_1.analyticsConfig.analyticsWindowMinutes}m. Review risk!`);
    }
    if (buckets.gt10 > 3) {
        alerts.push(`🔥 HOT STREAK: ${buckets.gt10} trades with PnL >10% in the last ${config_1.analyticsConfig.analyticsWindowMinutes}m!`);
    }
    console.log(`\n=== Test Case: ${name} ===`);
    console.log(`Summary:`);
