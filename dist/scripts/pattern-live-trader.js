...
if (!process.env.PATTERN_LIVE_TRADING_ENABLED ||
...
(async () => {
    try {
        ...
        if (!config || !config.trading || !config.solana) {
            ...
            console.error('[PatternLiveTrader] Troubleshooting: Check your .env file for missing or malformed variables.');
            process.exit(1);
        }
        if (!walletKeypair) {
            console.error('[PatternLiveTrader] Wallet keypair not loaded. Exiting.');
            console.error('[PatternLiveTrader] Troubleshooting: Ensure SOLANA_PRIVATE_KEY is set in your .env as a comma-separated number array.');
            process.exit(1);
        }
        ...
        const keypairs = keyArray.map((keyStr) => {
        ...
        const keyRotationManager = new KeyRotationManager(keypairs, Number(process.env.KEY_ROTATION_TRADES || 3));
        ...
        const engine = new TradingEngine(conn, config, walletKeypair, keyRotationManager, notifier.notifyInfo?.bind(notifier) // Alert function
        );
        ...
        whaleWatcher.on('whaleSignal', async (event) => {
            ...
            if (typeof orig === 'number')
                config.trading.pumpThreshold = orig * 0.5;
            setTimeout(() => {
                if (typeof orig === 'number')
                    config.trading.pumpThreshold = orig;
                pumpThresholds.delete(poolAddress);
            }, 30000);
            // Metric/alert
            if (notifier) {
                await notifier.notifyInfo(`[SECRET SAUCE] token=${poolAddress} event=whaleSignal usdcAmount=${usdcAmount}`);
            }
        ...
        congestionMonitor.on('congestion', async (event) => {
            ...
            if (notifier) {
                await notifier.notifyInfo(`[SECRET SAUCE] event=congestion blockTimeMs=${event.blockTimeMs} threshold=${event.threshold}`);
            }
            ...
            congestionTimeout = setTimeout(() => {
                ...
                if (typeof origMAX_HOLD_SEC === 'number')
                    config.trading.maxHoldSec = origMAX_HOLD_SEC;
        ...
        for await (const signal of trader.streamPatternSignals({ minutes: 1 })) {
            ...
            try {
                ...
                console.log(`${logPrefix} Pattern signal detected for ${signal.mint} at price ${signal.price}`);
                ...
                if (whaleSignal) {
                    console.log(`[SECRET SAUCE] Whale signal active for ${signal.mint} - lowering PUMP_THRESHOLD`);
                }
...
//# sourceMappingURL=pattern-live-trader.js.map