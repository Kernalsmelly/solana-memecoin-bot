...
class TxnBuilder {
    constructor(...args) { }
    async adjustPriorityFee(...args) { }
}
import { Connection } from '@solana/web3.js';
import { Keypair } from '@solana/web3.js';
export async function runSecretSaucePilot(config) {
    try {
        ...
        while (Date.now() < endTime && trades < config.maxTrades) {
            ...
            for (const signal of whaleSignals) {
                logger.info(`[SECRET SAUCE] Whale signal detected: ${signal.tokenMint} received ${signal.amount} USDC`);
...
//# sourceMappingURL=pilot_secret_sauce.js.map