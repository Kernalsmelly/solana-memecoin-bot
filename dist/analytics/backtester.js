...
class PatternBacktester {
    ...
    async runBacktest() {
        try {
            ...
            for (let i = 0; i < this.priceData.length; i++) {
                ...
                if (!currentPoint) {
                    logger_1.default.warn(`Skipping backtest iteration ${i} due to missing data point.`);
                    continue;
                }
    ...
    createSimulatedToken(index) {
        ...
        if (!currentPoint || prevPoints.length === 0) {
            logger_1.default.warn(`Skipping backtest iteration ${index} due to missing data point.`);
            return null;
        }
    ...
    calculateVolumeChange(prevPoints) {
        if (!prevPoints || prevPoints.length === 0)
            return 0;
        const lastPoint = prevPoints[prevPoints.length - 1];
        if (!lastPoint)
            return 0;
        const currentIndex = this.priceData.findIndex(p => p.timestamp === lastPoint.timestamp &&
            p.price === lastPoint.price &&
            p.volume === lastPoint.volume);
        if (currentIndex === -1 || currentIndex + 1 >= this.priceData.length)
            return 0;
    ...
    simulateTrade(pattern, startIndex) {
        try {
            ...
            if (entryPrice === undefined || entryPrice === null) {
                logger_1.default.warn(`Cannot execute trade for ${this.config.tokenAddress} at index ${startIndex}: entryPrice is undefined.`);
                return;
            }
