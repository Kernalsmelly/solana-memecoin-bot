...
async function launchTradingSystem() {
    ...
    try {
        ...
        if (keyStrs.length > 0) {
            ...
            keyRotationManager = new KeyRotationManager(keypairs, Number(process.env.TRADES_PER_KEY) || 1);
            wallet = keyRotationManager.nextKeypair();
            logger.info(`[KeyRotation] Loaded ${keypairs.length} keypairs for rotation. Using first keypair as wallet.`);
        }
        else {
        ...
        if (wssEndpoint)
            logger.info(`Using QuickNode WebSocket: ${wssEndpoint}`);
