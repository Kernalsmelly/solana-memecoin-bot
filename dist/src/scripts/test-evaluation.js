...
import { config } from '../utils/config.js'; // Correct: Import the pre-loaded config object and the type
...
async function runEvaluation() {
    ...
    try {
        ...
        if (KNOWN_QUOTE_MINTS.has(poolDetails.baseMint) &&
        ...
        else if (!KNOWN_QUOTE_MINTS.has(poolDetails.baseMint) &&
            KNOWN_QUOTE_MINTS.has(poolDetails.quoteMint)) {
        ...
        if (passesSecurityChecks) {
            ...
            if (!meetsLiquidityCriteria)
                logger.warn(`  Reason: Insufficient liquidity.`);
            if (!passesSecurityChecks)
                logger.warn(`  Reason: Failed security checks.`);
