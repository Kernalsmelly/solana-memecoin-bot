...
export class PatternDetector extends EventEmitter {
    ...
    constructor(config) {
        ...
        this.enabledPatterns =
            config.enabledPatterns || Object.keys(this.patternCriteria);
    ...
    setupEventListeners() {
        ...
        this.tokenDiscovery.on('tokenDiscovered', (token) => {
    ...
    analyzePatternMatch(token) {
        ...
        function detectVolatilitySqueeze(prices, currentPrice, period = 20, squeezeThreshold = 0.06) {
            ...
            const breakout = (typeof currentPrice === 'number' ? currentPrice : (prices?.[prices.length - 1] ?? 0)) >
                upper;
        ...
        for (const pattern of this.enabledPatterns) {
            ...
            if (pattern === 'Volatility Squeeze' &&
                Array.isArray(token.priceHistory) &&
                token.priceHistory.length >= (criteria.period || 20)) {
