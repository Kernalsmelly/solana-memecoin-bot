...
class LiveTradeSimulator extends events_1.EventEmitter {
    ...
    constructor() {
        ...
        this.newCoinDetector = new newCoinDetector_1.NewCoinDetector();
    }
    async start() {
    ...
    shouldMonitorNewToken(token) {
        ...
        if (token.initialLiquidity < 100000)
            return false;
        // Don't monitor too many tokens
        if (TOKENS_TO_MONITOR.length >= 10)
            return false;
        return true;
    }
    async fetchPriceData(token) {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                const jupiterResponse = await axios_1.default.get(`https://jup.ag/api/v4/price?ids=${token.address}&vsToken=USDC`, {
                    timeout: 5000
                });
                ...
                const dexResponse = await axios_1.default.get(`https://api.dexscreener.com/latest/dex/tokens/${token.address}`, {
                    headers: {
    ...
    shouldEnterPosition(token, history) {
        if (history.length < 5)
            return false;
        if (this.wallet.cash < 10)
            return false; // Minimum $10 for new positions
        if (this.wallet.positions.size >= this.maxPositions)
            return false;
        ...
        if (isNewToken) {
            return (priceChange > 1 && // Lower threshold for new tokens
        ...
        return (priceChange > 2 && // >2% price increase
    ...
    takeProfits(position, sellQuantity, level) {
