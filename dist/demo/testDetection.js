...
class DetectionTester {
    constructor() {
        this.tokenCount = 0;
        this.symbols = ['DOGE', 'CAT', 'PEPE', 'MOON', 'SHIB', 'FLOKI', 'PUDGY', 'BONK', 'BOOK', 'RACA'];
        this.namePrefixes = ['Super', 'Mega', 'Ultra', 'Hyper', 'Cosmic', 'Galactic', 'Atomic', 'Solar', 'Lunar', 'Stellar'];
        this.nameSuffixes = ['Token', 'Coin', 'Inu', 'Moon', 'Rocket', 'Elon', 'Finance', 'Protocol', 'AI', 'Swap'];
        // Initialize with standard testing parameters
        this.detector = new newCoinDetector_1.NewCoinDetector({
            minLiquidity: 5000,
            maxAgeHours: 72,
            scanIntervalSec: 10,
        ...
        this.detector.on('newToken', async (token) => {
    ...
    createRandomToken() {
        ...
        const address = (0, uuid_1.v4)().replace(/-/g, '').substring(0, 44); // Solana-like address
        const metrics = {
            address,
            symbol,
     ...
     */
    updateToken(address, priceChange, volumeChange, buyRatioChange) {
    ...
    async runTestScenario() {
        ...
        for (const test of patternTests) {
            const { address } = this.addPatternToken(test.type); // Use type assertion
