import { MockPriceFeed, VolumeProfile } from '../utils/mockPriceFeed.js';
...
async function runRobustTest() {
  ...
  for (const scenario of scenarios) {
    ...
    if (scenario.patterns.length > 0) {
      ...
      if (firstPattern) {
        ...
        console.error(
          `Scenario "${scenario.name}" has patterns array but first element is undefined.`,
        );
        continue;
      }
    } else {
    ...
    for (const pattern of scenario.patterns) {
      ...
      console.log(`\n📊 ${pattern.name}:`);
      console.log(
        `Price: $${pattern.price.toFixed(3)} | Volume: ${(pattern.volume / 1000).toFixed(1)}K`,
      );
      console.log(`P&L: ${pnlPercent.toFixed(1)}% | DD: ${drawdown.toFixed(1)}%`);
      console.log(
        `Momentum: ${pattern.momentum.toFixed(1)} | Volume Trend: ${pattern.volumeTrend.toFixed(1)}%`,
      );
    }
  ...
  scenarios.forEach((scenario, i) => {
    ...
    if (result) {
      console.log(
        `${scenario.name}: ${result.pnl.toFixed(1)}% (DD: ${result.maxDrawdown.toFixed(1)}%)`,
      );
    } else {
