...
import {
  MultiSourceTokenDetector,
  MultiSourceTokenEvent,
} from './/services/multiSourceTokenDetector.js';
...
import { ExitManager, ExitManagerConfig, ManagedPosition } from './strategy/exitManager.js';
import { saveOpenPositions, loadOpenPositions } from './utils/persistence.js';
import { PortfolioOptimizer } from './strategy/portfolioOptimizer.js';
import logger from './utils/logger.js';

import { config } from './utils/config.js';
import verifyConfig from './utils/verifyConfig.js';
import runPreFlightCheck from './utils/preFlightCheck.js';
import { sendAlert } from './utils/notifications.js';
import { Config } from './utils/config.js'; // Keep Config type import if needed elsewhere
// import { Config } from './utils/config.js'; // Keep Config type import if needed elsewhere
import { PatternDetection, Position, TradeOrder, OrderExecutionResult } from './/types.js';
...
async function main() {
  ...
  try {
    ...
    mempoolSniffer.on('mintDetected', async (mintAddress: string, tx: any) => {
      try {
        ...
        if (config.trading.forcePumpOnMempool) {
          logger.info(
            `[ForcedPump] Attempting forced pump for ${mintAddress} due to mempool event`,
          );
          forcedPumpManager
            .maybeForcePump(new PublicKey(mintAddress), false)
            .then((executed: boolean) => {
              if (executed) logger.info(`[ForcedPump] Forced pump executed for ${mintAddress}`);
            });
        }
    ...
    mempoolSniffer.on('lpAddDetected', async (lpAddress: string, mintAddress: string, tx: any) => {
      try {
    ...
    mempoolSniffer.start().catch((e: any) => logger.error('[MempoolSniffer] Failed to start:', e));
    ...
    whaleSignalWatcher.on('whaleSignal', (event: any) => {
      try {
        logger.info(
          `[WhaleSignal] Whale trade detected: ${event.usdcAmount} USDC to pool ${event.poolAddress}`,
        );
        ...
        if (config.trading.forcePumpOnWhale) {
          logger.info(
            `[ForcedPump] Attempting forced pump for ${event.tokenMint} due to whale event`,
          );
          forcedPumpManager
            .maybeForcePump(new PublicKey(event.tokenMint), false)
            .then((executed: boolean) => {
              if (executed) logger.info(`[ForcedPump] Forced pump executed for ${event.tokenMint}`);
            });
        }
        // Synthesize a MultiSourceTokenEvent for the pipeline
        multiSourceTokenDetector.emit('newTokenDetected', {
    ...
    const priceWatcher = new (await import('./services/priceWatcher.js')).PriceWatcher(
      connection,
      config,
    );
    ...
    try {
      ...
      multiSourceTokenDetector.on('newTokenDetected', async (evt: MultiSourceTokenEvent) => {
        logger.info(
          `[MultiSourceTokenDetector] New token detected from ${evt.source}: ${evt.mint} (${evt.symbol || ''})`,
        );
        ...
        if (
          ...
          logger.info(
            `[NewPoolDetected] No strong pattern detected for ${evt.metrics?.symbol || detectedPattern?.baseMint}`,
          );
          return;
        }
        ...
        if (newPosition) {
          logger.info(
            `[NewPoolDetected] Opened position for ${evt.metrics?.symbol || detectedPattern.baseMint} (${evt.metrics?.address || detectedPattern.baseMint})`,
          );
          ...
          const quantityDecimal =
            Number(newPosition.quantity) / Math.pow(10, newPosition.tokenDecimals);
          ...
          logger.info(
            `PortfolioOptimizer did not open a position for pattern ${detectedPattern.pattern} on ${evt.metrics?.symbol || detectedPattern.baseMint}.`,
          );
        }
      });
    ...
    try {
      ...
      exitManager.on(
        ...
        (
          ...
          logger.info(
            `<<< ExitManager closed position: ${position.tokenSymbol}, Reason: ${reason}`,
          );
          logger.info(
            `<<< ExitManager closed position: ${position.tokenSymbol}, Reason: ${reason}`,
          );
            ...
            if (position.initialSolCostLamports !== undefined) {
              ...
              pnlPercent =
                initialSolCostLamports > 0n
                  ...
                  : 0;
              logger.info(
                `Trade closed successfully. PNL: ${pnlLamports} lamports (${pnlPercent.toFixed(2)}%). Tx: ${executionResult.txSignature}`,
              );
            } else {
              logger.warn(
                `Cannot calculate SOL-based PNL for ${position.id}, initialSolCostLamports is missing. Reporting 0% PNL.`,
              );
            }
          }
      ...
      await sendAlert(
        `[MAIN] Unhandled error in main(): ${err instanceof Error ? err.message : String(err)}`,
        'CRITICAL',
      );
    }
  } catch (err) {
