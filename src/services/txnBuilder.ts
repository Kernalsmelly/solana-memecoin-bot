...
const Prometheus = {
  registerCounter: (..._args: any[]) => ({ inc: (..._a: any[]) => {} }),
  registerHistogram: (..._args: any[]) => ({ observe: (..._a: any[]) => {} }),
};

// Prometheus metrics for transaction performance
const priorityFeeSaves = Prometheus.registerCounter(
  'priority_fee_saves_total',
  'Number of priority fee saves',
);
const txSendLatency = Prometheus.registerHistogram(
  'tx_send_latency_ms',
...
export class TxnBuilder {
  constructor(connection: Connection, options: Partial<TxnBuilderOptions> = {}) {
    ...
    if (isNaN(envFee)) envFee = 0.0002;
    this.options = {
      priorityFee:
        typeof options.priorityFee === 'number' && !isNaN(options.priorityFee)
          ? options.priorityFee
  ...
  private options: TxnBuilderOptions;
  private lastPriorityFee: number = 0;

  // In-memory template store: { [poolKey]: { buy: Transaction, sell: Transaction } }
  private txTemplates: Record<string, { buy?: Transaction; sell?: Transaction }> = {};
   ...
   */
  public async buildTemplateTx(
    poolInfo: { poolKey: string; inputMint: string; outputMint: string },
    type: 'buy' | 'sell',
    signers: Keypair[],
  ): Promise<void> {
    ...
    tx.sign(...signers);
    // Store template
    if (!this.txTemplates[poolKey]) this.txTemplates[poolKey] = {};
   ...
   */
  public async fillAndSendTemplateTx(
    poolKey: string,
    type: 'buy' | 'sell',
    amount: number,
    fee: number,
    ...
    if (!template) throw new Error(`No ${type} template found for pool ${poolKey}`);
  ...
  public async buildAndSend(
    ...
    try {
      ...
      Prometheus.registerCounter(
        'priority_fee_adjustments_total',
        'Number of priority fee adjustments',
      ).inc();
      logger.info(
        `[TxnBuilder] Called adjustPriorityFee with latency ${latencyMs}ms after send/confirm.`,
      );
      if (latencyMs > 2000) {
        Prometheus.registerCounter(
      ...
      while (!confirmed && retries < this.options.maxRetries) {
        try {
          const confirmation: { value: { err: null | object } } = await this.connection.confirmTransaction(signature) as { value: { err: null | object } };
          // Type guard for confirmation result
          confirmed = Boolean(
            confirmation && confirmation.value && confirmation.value.err === null,
          );
        } catch (error) {
          retries++;
  ...
  public async adjustPriorityFee(currentLatency: number): Promise<void> {
    ...
    if (Math.abs(adjustedFee - this.options.priorityFee) > 0.00001) {
      logger.info(
        `[TxnBuilder] Adjusting priority fee: ${this.options.priorityFee} → ${adjustedFee}`,
      );
