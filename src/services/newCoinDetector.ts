...
import { TokenMetrics, PatternDetection, TradingSignal } from '../types.js';
import {
  PublicKey,
  Connection,
  ParsedAccountData,
...
} from '@solana/web3.js'; // Ensure PublicKey & ParsedAccountData are imported
...
import { fetchDexscreenerPoolData } from './dexscreener.js';
...
function isParsedInstructionWithInitializeInfo(
  ...
  parsed: { type: string | 'initialize2'; info: Initialize2Info };
} {
  return (
    instruction &&
...
function isMintAccountInfoWithDecimals(
  ...
  value: { data: ParsedAccountData & { parsed: { type: 'mint'; info: { decimals: number } } } };
} {
  return (
    accountInfo &&
...
export class NewCoinDetector extends EventEmitter {
    ...
    setInterval(
      () => {
        ...
        if (oldSize > this.MAX_PROCESSED_POOLS / 2) {
          ...
          logger.debug(
            `[NewCoinDetector] Cleaned processed pools cache from ${oldSize} to ${this.processedPools.size} entries`,
          );
        }
      },
      30 * 60 * 1000,
  ...
  public start(): void {
    ...
    logger.info(
      `[NewCoinDetector] Using poll interval of ${pollInterval / 1000} seconds with max ${this.MAX_RPC_CALLS_PER_MINUTE} RPC calls/min`,
    );
    logger.info(
      `[NewCoinDetector] Processing max ${this.MAX_POOLS_TO_PROCESS} pools per cycle with ${this.DELAY_BETWEEN_POOLS_MS}ms delay between pools`,
    );
    ...
    this.pollIntervalId = setInterval(() => {
      ...
      if (rpcUtilization > 0.8 || globalUtilization > 0.8) {
        logger.warn(
          `[RateLimit] Skipping NewCoinDetector poll cycle due to high API utilization - Local: ${(rpcUtilization * 100).toFixed(0)}%, Global: ${(globalUtilization * 100).toFixed(0)}%`,
        );
        return;
      }
  ...
  private async pollOnChain(): Promise<void> {
    ...
    try {
      ...
      if (pools.length > 0) {
        logger.info(
          `[OnChain] Found ${pools.length} pools, processing max ${this.MAX_POOLS_TO_PROCESS}`,
        );
  ...
  private checkRpcRateLimit(): boolean {
    ...
    if (this.rpcCallCount >= this.MAX_RPC_CALLS_PER_MINUTE) {
      logger.warn(
        `[RateLimit] NewCoinDetector local RPC call limit reached (${this.MAX_RPC_CALLS_PER_MINUTE}/min). Throttling.`,
      );
      return false;
    }
  ...
  private async pollForNewPools(): Promise<void> {
    ...
    logger.info(
      `[Polling] Starting Raydium pool poll... RPC utilization: Local ${localUtilization.toFixed(0)}%, Global ${globalUtilization}%`,
    );
    ...
    try {
      ...
      if (raydiumPools && raydiumPools.length > 0) {
        logger.info(
          `[Polling] Found ${raydiumPools.length} pools from Raydium API, processing max ${this.MAX_POOLS_TO_PROCESS}`,
        );
        ...
        for (const pool of poolsToProcess) {
          ...
          if (!this.checkRpcRateLimit()) {
            logger.warn(
              `[RateLimit] Pausing pool processing due to rate limiting. Processed ${processedCount} of ${poolsToProcess.length} pools.`,
            );
            break;
          }
        ...
        logger.info(
          '[Polling] No new pools found from Raydium API, falling back to on-chain polling.',
        );
      ...
      logger.debug(
        `[Polling] Completed poll cycle. RPC utilization: Local ${finalLocalUtilization.toFixed(0)}%, Global ${finalGlobalUtilization}%`,
      );
    }
  }
  ...
  private async processSignature(signature: string): Promise<void> {
    ...
    try {
      ...
      const ix = tx.transaction.message.instructions.find((i) => {
        if ('data' in i && 'programId' in i) {
          try {
            ...
            return (
              programId.equals(RAYDIUM_LP_V4_PROGRAM_ID) &&
      ...
      const accountKey = accountKeys ? accountKeys[(ix as any).accounts[0]] : null;
