...
import { logPriceHistory } from '../utils/priceHistoryLogger.js';
...
export class PriceWatcher extends EventEmitter {
  ...
  private async cacheQuoteTokenDecimals() {
    ...
    const quoteMints = [SOL_MINT, this.config.solana.usdcMint, USDT_MINT].filter((mint) => !!mint);
    ...
    for (const mint of quoteMints) {
      if (mint) {
        try {
          ...
          logger.warn(
            `[PriceWatcher] Failed to pre-cache decimals for quote mint ${mint}: ${error}`,
          );
        }
      }
    }
  ...
  private async getTokenDecimals(mintAddress: string): Promise<number> {
    ...
    if (!this.checkRpcRateLimit()) {
      logger.warn(
        `[PriceWatcher] RPC rate limit reached, using default decimals for ${mintAddress}`,
      );
    ...
    try {
      ...
      if (
        ...
        parsedData !== null &&
        'info' in parsedData &&
        typeof parsedData.info === 'object' &&
        parsedData.info !== null &&
        'decimals' in parsedData.info &&
        typeof parsedData.info.decimals === 'number'
      ) {
        ...
        logger.warn(`[PriceWatcher] Parsed account info structure invalid for ${mintAddress}`, {
          parsedData,
        });
  ...
  start() {
    ...
    logger.info(
      `Max RPC calls: ${this.MAX_RPC_CALLS_PER_MINUTE}/min, Max Jupiter calls: ${this.MAX_JUPITER_CALLS_PER_MINUTE}/min`,
    );
    ...
    this.pollIntervalId = setInterval(() => {
      ...
      if (rpcUtilization > 0.9 || jupiterUtilization > 0.9) {
        logger.warn(
          `[RateLimit] Skipping poll cycle due to high API utilization - RPC: ${(rpcUtilization * 100).toFixed(0)}%, Jupiter: ${(jupiterUtilization * 100).toFixed(0)}%`,
        );
        return;
      }
      ...
      this.pollWatchedTokens().catch((error) => {
        ...
        logger.error(
          `[PriceWatcher] Uncaught error during pollWatchedTokens execution: ${error.message}`,
        );
      });
    }, this.pollIntervalMs);
  }
  ...
  watchToken(mintAddress: string, pairAddress?: string) {
    if (!this.watchedTokens.has(mintAddress)) {
      logger.info(
        `[PriceWatcher] Starting to watch token: ${mintAddress}${pairAddress ? ` (Pair: ${pairAddress})` : ''}`,
      );
      ...
      if (existingData && !existingData.pairAddress && pairAddress) {
        ...
        logger.info(
          `[PriceWatcher] Updated pair address for already watched token ${mintAddress} to ${pairAddress}`,
        );
      }
    }
  }
  ...
  private checkRpcRateLimit(): boolean {
    ...
    if (this.rpcCallCount >= this.MAX_RPC_CALLS_PER_MINUTE) {
      logger.warn(
        `[RateLimit] RPC call limit reached (${this.MAX_RPC_CALLS_PER_MINUTE}/min). Throttling.`,
      );
      return false;
    }
  ...
  private checkJupiterRateLimit(): boolean {
    ...
    if (this.jupiterCallCount >= this.MAX_JUPITER_CALLS_PER_MINUTE) {
      logger.warn(
        `[RateLimit] Jupiter API call limit reached (${this.MAX_JUPITER_CALLS_PER_MINUTE}/min). Throttling.`,
      );
      return false;
    }
  ...
  private async pollWatchedTokens() {
    ...
    if (rpcUtilization > 0.8 || jupiterUtilization > 0.8) {
      logger.warn(
        `[RateLimit] High API utilization - RPC: ${(rpcUtilization * 100).toFixed(0)}%, Jupiter: ${(jupiterUtilization * 100).toFixed(0)}%. Reducing polling.`,
      );
    ...
    logger.debug(
      `[PriceWatcher] Polling ${this.watchedTokens.size} tokens. RPC calls: ${this.rpcCallCount}/${this.MAX_RPC_CALLS_PER_MINUTE}, Jupiter: ${this.jupiterCallCount}/${this.MAX_JUPITER_CALLS_PER_MINUTE}`,
    );
    const pollPromises = Array.from(this.watchedTokens.keys()).map((mint) =>
      this.pollSingleToken(mint),
    );
    ...
    results.forEach(async (result, index) => {
      if (result.status === 'rejected') {
        ...
        await sendAlert(
          `[PriceWatcher] Polling promise rejected for ${mint}: ${result.reason}`,
          'CRITICAL',
        );
      }
    });
  ...
  private async pollSingleToken(mintAddress: string) {
    ...
    try {
      ...
      if (tokenData.pairAddress) {
        ...
        if (!pairData && tokenData.errorCount < this.maxPollingErrors) {
          // If fetch by pair fails, try token search once?
          logger.warn(
            `[PriceWatcher] Failed to fetch Dexscreener by pair ${tokenData.pairAddress}, trying token search for ${mintAddress}`,
          );
          pairData = await this.fetchDexscreenerData(mintAddress, 'token');
        }
      } else {
        logger.debug(
          `[PriceWatcher] No pair address for ${mintAddress}, attempting Dexscreener search by token...`,
        );
        pairData = await this.fetchDexscreenerData(mintAddress, 'token');
        // If search finds a pair, update our stored data
        if (pairData?.pairAddress && !tokenData.pairAddress) {
          logger.info(
            `[PriceWatcher] Found pair address ${pairData.pairAddress} for ${mintAddress} via token search.`,
          );
      ...
      if (pairData?.volume?.h1 !== undefined) {
        // Example: track h1 volume
      ...
      const volumeChangePercent = this.calculateVolumeChangePercent(
        tokenData.volumeHistory,
        15 * 60 * 1000,
      ); // 15 minutes
      ...
      await sendAlert(
        `[PriceWatcher] Error polling token ${mintAddress}: ${error.message}`,
        'CRITICAL',
      );
      ...
      logger.warn(
        `[PriceWatcher] Consecutive error count for ${mintAddress}: ${tokenData.errorCount}`,
      );
      if (tokenData.errorCount >= this.maxPollingErrors) {
        logger.error(
          `[PriceWatcher] Exceeded max polling errors (${this.maxPollingErrors}) for ${mintAddress}. Unwatching.`,
        );
  ...
  private async fetchJupiterPrice(
    ...
    if (!this.checkJupiterRateLimit()) {
      logger.warn(
        `[PriceWatcher] Jupiter rate limit reached for ${inputMint}, using cached price if available`,
      );
    ...
    try {
      ...
      if (!quoteResponse || !quoteResponse.outAmount || quoteResponse.outAmount === '0') {
        throw new Error(
          `Invalid quote response or zero outAmount from Jupiter V6 for ${inputMint}`,
        );
      }
      ...
      logger.error(
        `[PriceWatcher] Jupiter quote failed for ${inputMint} -> ${outputMint}: ${errorMessage}`,
      );
      await sendAlert(
        `[PriceWatcher] Jupiter quote failed for ${inputMint} -> ${outputMint}: ${errorMessage}`,
        'ERROR',
      ...
      if (tokenData?.lastPrice) {
        logger.info(
          `[PriceWatcher] Using cached price ${tokenData.lastPrice} for ${inputMint} due to Jupiter error`,
        );
        return tokenData.lastPrice;
      }
  ...
  async fetchDexscreenerData(queryAddress: string, type: 'pair' | 'token'): Promise<any> {
    ...
    try {
      ...
      if (response.status !== 200) {
        logger.warn(
          `[PriceWatcher] Dexscreener API returned non-200 status: ${response.status} for ${url}`,
        );
        return null;
      }
      ...
      if (type === 'token') {
        ...
        targetPair = pairs
          ...
          .find(
            (p: any) =>
              p.quoteToken?.address === this.config.solana.usdcMint ||
              p.quoteToken?.symbol === 'SOL',
          );
        if (!targetPair) {
          targetPair = pairs[0];
          logger.debug(
            `[PriceWatcher] No primary quote pair (USDC/SOL) found via token search for ${queryAddress}, using most liquid: ${targetPair?.pairAddress}`,
          );
        } else {
          logger.debug(
            `[PriceWatcher] Found target pair ${targetPair.pairAddress} via token search for ${queryAddress}`,
          );
        }
      } else {
        targetPair = pairs[0];
      ...
      if (!targetPair) {
        logger.warn(
          `[PriceWatcher] Could not determine target pair from Dexscreener response for ${url}`,
        );
        return null;
      }
      return targetPair;
      ...
      if (axiosError.isAxiosError) {
        logger.error(
          `[PriceWatcher] Axios error fetching Dexscreener data for ${url}: ${axiosError.message}`,
          {
  ...
  private calculatePriceChange(
    ...
    const sortedHistory = history
      .filter(
        (entry) => entry && typeof entry.timestamp === 'number' && typeof entry.price === 'number',
      )
    ...
    if (!isFinite(priceChange)) {
      logger.warn(`[PriceWatcher] Calculated infinite price change`, {
        current: current.price,
        start: startEntry.price,
      });
      return undefined;
    }
  ...
  private calculateVolumeChangePercent(
    ...
    const startEntry = history.find(
      (entry) => entry && typeof entry.timestamp === 'number' && entry.timestamp > now - durationMs,
    );
  ...
  private trimHistory(history: { timestamp: number; [key: string]: any }[], maxAgeMs: number) {
    ...
    const startIndex = history.findIndex(
      (entry) => entry && typeof entry.timestamp === 'number' && entry.timestamp >= cutoffTimestamp,
    );

    if (startIndex > 0) {
      // Remove elements before the startIndex
      history.splice(0, startIndex);
    } else if (startIndex === -1 && history.length > 0) {
