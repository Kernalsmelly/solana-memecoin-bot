...
export class WhaleSignalDetector extends EventEmitter {
  ...
  private options: WhaleSignalOptions;
  private recentSignals: Map<string, number> = new Map(); // tokenMint -> signal timestamp
  ...
  private processAccountChange(accountInfo: any, context: any): void {
  ...
  private parseTransferAmount(data: Buffer): number | null {
    try {
  ...
  private extractTokenMint(data: Buffer): string | null {
    try {
  ...
  private handleWhaleSignal(tokenMint: string, amount: number): void {
    // Record the signal
    ...
    // Log the event
    logger.info(
      `[WhaleSignalDetector] Whale signal detected: ${tokenMint} received ${amount} USDC`,
    );
  }

  public hasRecentSignal(tokenMint: string): boolean {
