...
export class MultiSourceTokenDetector extends EventEmitter {
          ...
          try {
            ...
            if (msg.type === 'new_pool' && msg.data?.mint) {
              if (!this.watchedMints.has(msg.data.mint)) {
                ...
                logger.info(
                  `[MultiSourceTokenDetector] Birdeye detected new token: ${msg.data.mint}`,
                );
              }
            }
          } catch (e) {
  ...
  private async pollJupiter() {
    try {
      ...
      for (const t of tokens) {
        logger.debug(
          `[MultiSourceTokenDetector] Jupiter considering token: ${t.address} (${t.symbol || ''})`,
        );
        if (t.address && !this.watchedMints.has(t.address)) {
          ...
          try {
            ...
            logger.info(
              `[MultiSourceTokenDetector] Scored ${metrics?.symbol || t.symbol} (${t.address}): ${score} [${reasons.join(', ')}]`,
            );
            if (score >= 50) {
              ...
              logger.debug(
                `[MultiSourceTokenDetector] Skipping ${metrics?.symbol || t.symbol} (${t.address}) due to low score (${score})`,
              );
            }
          } catch (err) {
            if (err instanceof Error) {
              logger.warn(
                `[MultiSourceTokenDetector] Error scoring token ${t.address}: ${err.message}`,
              );
            } else {
              logger.warn(
                `[MultiSourceTokenDetector] Error scoring token ${t.address}: ${String(err)}`,
              );
            }
          }
        }
  ...
  private async pollDexscreener() {
    ...
    try {
      ...
      for (const t of tokens) {
        logger.debug(
          `[MultiSourceTokenDetector] Dexscreener considering token: ${t.address} (${t.symbol || ''})`,
        );
        if (t.address && !this.watchedMints.has(t.address)) {
          // Score the token before emitting
          try {
            ...
            if (score >= 50) {
              ...
              logger.info(
                `[MultiSourceTokenDetector] Dexscreener detected new token: ${t.address}`,
              );
            } else {
              logger.debug(
                `[MultiSourceTokenDetector] Skipping ${metrics?.symbol || t.symbol} (${t.address}) due to low score (${score})`,
            ...
            if (err instanceof Error) {
              logger.warn(
                `[MultiSourceTokenDetector] Error scoring token ${t.address}: ${(err as Error).message}`,
              );
            } else {
              logger.warn(
                `[MultiSourceTokenDetector] Error scoring token ${t.address}: ${String(err)}`,
