...
import { Connection, Keypair, PublicKey, LAMPORTS_PER_SOL } from '@solana/web3.js'; // Added PublicKey and LAMPORTS_PER_SOL for SOL conversion
import bs58 from 'bs58';
import * as fs from 'fs';
import * as path from 'path';
import logger from './utils/logger.js';
import { LiveOrderExecution, OrderExecutionConfig } from './orderExecution.js';
...
import { KeyRotationManager } from './utils/keyRotationManager.js';
import { RiskMetrics } from './types.js';
import { NewCoinDetector, NewPoolDetectedEvent } from './services/newCoinDetector.js';
import { PriceWatcher } from './services/priceWatcher.js';
...
class RpcRateLimiter {
  ...
  private constructor() {
    ...
    setInterval(() => {
      ...
      logger.debug(
        `[RateLimiter] Reset RPC call counter. Current utilization: ${this.getUtilizationPercent()}%`,
      );
    }, 60000);
  }
  ...
  public checkLimit(): boolean {
    if (this.rpcCallCount >= this.MAX_RPC_CALLS_PER_MINUTE) {
      logger.warn(
        `[RateLimiter] RPC call limit reached (${this.MAX_RPC_CALLS_PER_MINUTE}/min). Throttling.`,
      );
      return false;
    }
    this.rpcCallCount++;
...
const PRODUCTION_MODE =
  process.env.NODE_ENV === 'production' || process.env.PRODUCTION_MODE === 'true';
const DRY_RUN_MODE = process.argv.includes('--dry-run') || process.env.DRY_RUN_MODE === 'true';
...
function loadSystemState(stateFilePath: string): Partial<RiskMetrics> | null {
  if (fs.existsSync(stateFilePath)) {
    try {
      ...
      logger.info(`Loaded system state from ${stateFilePath}`, {
...
async function launchTradingSystem() {
  ...
  try {
    ...
    // --- Multi-key support: Load from SOLANA_PRIVATE_KEYS (comma-separated base58) ---
    /**
     * To enable multi-key rotation, set SOLANA_PRIVATE_KEYS in .env as a comma-separated list of base58 secrets.
     * Example: SOLANA_PRIVATE_KEYS=key1,key2,key3
     * If not set, fallback to SOLANA_PRIVATE_KEY for single-key mode.
     */
    let keyRotationManager: KeyRotationManager | null = null;
    let wallet: Keypair;
    const keyStrs = (process.env.SOLANA_PRIVATE_KEYS || '')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
    if (keyStrs.length > 0) {
      const keypairs = keyStrs.map((base58key) => Keypair.fromSecretKey(bs58.decode(base58key)));
      keyRotationManager = new KeyRotationManager(
        keypairs,
        Number(process.env.TRADES_PER_KEY) || 1,
      );
      wallet = keyRotationManager.nextKeypair();
      logger.info(
        `[KeyRotation] Loaded ${keypairs.length} keypairs for rotation. Using first keypair as wallet.`,
      );
    } else {
    ...
    logger.info(
      `[RateLimiter] Using global rate limiter with max ${globalRateLimiter.getMaxCount()} RPC calls per minute`,
    );
    logger.warn(
      'IMPORTANT: Monitor QuickNode usage carefully to avoid exceeding your plan limits.',
    );
    ...
    if (!wallet) {
      // Ensure wallet is loaded before passing
    ...
    riskManager.on('circuitBreaker', async (data) => {
      ...
      await sendAlert(
        `Circuit breaker triggered: ${reason} - ${message ?? 'No details'}`,
        'WARNING',
      ); // Added nullish coalescing for message
    ...
    riskManager.on('emergencyStop', async (data) => {
      ...
      await sendAlert(
        `🚨 EMERGENCY STOP ACTIVATED: ${reason} - ${message ?? 'No details'}`,
        'CRITICAL',
      ); // Added nullish coalescing for message
    ...
    const orderExecutionConfig: OrderExecutionConfig = {
      slippageBps: config.trading.jupiterSlippageBps,
    };
    ...
    newCoinDetector.on('newPoolDetected', (eventData: NewPoolDetectedEvent) => {
      logger.info(
        `[Launch] Detected new pool via event: Base Mint ${eventData.baseMint}, Pool ${eventData.poolAddress}, Quote ${eventData.quoteMint}`,
      );
      ...
      if (priceWatcher) {
        ...
        logger.error(
          '[Launch] PriceWatcher not initialized when newPoolDetected event was received. Cannot watch token.',
        );
      }
    ...
    if (priceWatcher && tradingEngine) {
      ...
      priceWatcher.on('marketDataUpdate', (marketData) => {
        // tradingEngine?.evaluateToken(marketData); // TODO: Uncomment if evaluateToken is implemented
      });
    } else {
      logger.error(
        '[Launch] Failed to connect PriceWatcher and TradingEngine. One or both are not initialized.',
      );
    }
    ...
    if (DRY_RUN_MODE) {
      ...
      logger.info('Dry run mode active. Monitoring for trading opportunities...');
    } else {
      ...
      // === Forced Trade Manual Trigger Integration ===
      // Only run this loop in PRODUCTION_MODE
      ...
      let forcedTradeActive = true;
      (async function forcedTradePollingLoop() {
        while (forcedTradeActive && !DRY_RUN_MODE) {
          try {
            if (await checkForManualTrigger()) {
              logger.info(`[FORCED_TRADE] [${new Date().toISOString()}] Detected force_trade.txt, executing forced BONK trade: ${bonkMint}`);
              if (tradingEngine) {
                try {
                  await tradingEngine.buyToken(new PublicKey(bonkMint));
                  logger.info(`[FORCED_TRADE] Executed forced BONK trade on ${bonkMint}`);
                } catch (err) {
                  logger.error(`[FORCED_TRADE] Error executing forced BONK trade:`, err);
                }
              } else {
                logger.error('[FORCED_TRADE] TradingEngine not initialized, cannot execute forced trade.');
              }
            }
          } catch (err) {
            logger.error('[FORCED_TRADE] Error in forced trade polling loop:', err);
          }
          await new Promise(res => setTimeout(res, 5000));
        }
      })();
      // === End Forced Trade Manual Trigger Integration ===
      ...
      const healthCheckIntervalId = setInterval(
        async () => {
          try {
            ...
            if (tradingEngine) {
              // const activePositions = tradingEngine.getPositions(); // TODO: Uncomment if getPositions is implemented
              // logger.info(`Active positions: ${activePositions.length}`); // TODO: Uncomment if getPositions is implemented
            } else {
            ...
            // Log RPC usage stats
            logger.info(
              `[RateLimiter] Current RPC utilization: ${globalRateLimiter.getUtilizationPercent()}% (${globalRateLimiter.getCurrentCount()}/${globalRateLimiter.getMaxCount()} calls)`,
            );
          } catch (error) {
...
// Run when invoked directly (ESM compatible)
