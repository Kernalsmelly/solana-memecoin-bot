...
interface JupiterApiClient {
  ...
  swapPost(params: { swapRequest: SwapRequest }): Promise<SwapResponse>;
}
...
export class JupiterDex {
  ...
  private async validatePositionSize(quote: QuoteResponse, inputPriceUsd: number): Promise<void> {
    try {
      ...
      const inAmountLamports = (Decimal as any)(quote.inAmount); // Comes as string
      const inputDecimals = await this.getTokenDecimals(inputMint);

      const inAmountTokens = inAmountLamports.div((Decimal as any)(10).pow(inputDecimals));
      ...
      const msg =
        error instanceof Error ? error.message : 'Unknown error during position size validation';
  ...
  public async getQuote(
    ...
    try {
      ...
      try {
        ...
        if (!this.jupiterApi) {
          throw new JupiterError(
            JupiterErrorType.API_CLIENT_NOT_INITIALIZED,
            'Jupiter API client is not initialized',
          );
        }
        ...
        const divisor = (Decimal as any)(10).pow(outputDecimals);
        const outAmountDecimal = (Decimal as any)(String(quoteResponse.outAmount)).div(divisor);
        const rawAmountDecimal = (Decimal as any)(String(rawAmount)); // Convert number to string before creating Decimal
        ...
        if (inputMintForValidation) {
          // Check if inputMint was assigned
          // Assuming input is always SOL (or wrapped SOL) for initial purchase/swap
          const inputMintForValidation =
            inputMint === 'So11111111111111111111111111111111111111112'
              ? inputMint
              : 'So11111111111111111111111111111111111111112'; // Use native SOL for price check
          ...
          if (inputPriceUsd > 0 && outputPriceUsd > 0) {
            ...
            logger.warn(
              `Could not get price for input token ${inputMintForValidation} to validate position size.`,
            );
          ...
          logger.warn(
            'Input mint not available from quote response, skipping position size validation.',
          );
        }
        ...
        if (this.riskManager && executionId) {
          this.riskManager.completeTradeExecution(
            executionId,
            false,
  ...
  public async executeSwap(
    ...
    if (!this.connection) {
      throw new JupiterError(
        JupiterErrorType.CONNECTION_NOT_INITIALIZED,
        'Solana connection is not initialized',
      );
    }
    ...
    try {
      ...
      if (signedTxs.length === 0) {
        throw new JupiterError(
          JupiterErrorType.NO_TRANSACTION_RETURNED,
          'No swap transactions returned from Jupiter API',
        );
      }
      ...
      for (const signedTx of signedTxs) {
        ...
        if (confirmation.value.err) {
          logger.error(`Transaction ${txid} confirmation failed`, {
            error: confirmation.value.err,
          });
          throw new JupiterError(
            JupiterErrorType.TRANSACTION_CONFIRMATION_FAILURE,
            `Transaction ${txid} failed to confirm`,
      ...
      if (error.name === 'WalletSignTransactionError' || error.message?.includes('User rejected')) {
        throw new JupiterError(
          JupiterErrorType.WALLET_SIGN_REJECTED,
      ...
      // Default error
      throw new JupiterError(
        JupiterErrorType.SWAP_EXECUTION_FAILED,
        error.message || 'Failed to execute swap',
        error,
      );
    }
  ...
  public async getTokenBalance(tokenMint: string): Promise<number> {
    try {
      ...
      const balance = (Decimal as any)(account.account.data.parsed.info.tokenAmount.uiAmountString);
