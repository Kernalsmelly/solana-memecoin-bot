...
// import { VolumeSpike } from '../strategies/volumeSpike.js'; // FIXME: File missing or not implemented
// import { ParameterUpdateEvent } from '../metrics/parameterMetrics.js'; // Not exported
import { PriceFeedManager, OHLCVEvent } from '../utils/priceFeedManager.js';
...
export interface BacktestResult {
  strategy: string;
  params: { priceChangeThreshold: number; volumeMultiplier: number };
...
export class Trader {
  ...
  private patternOnly: boolean;

  constructor(conn: Connection, opts: { patternOnly?: boolean } = {}) {
    this.conn = conn;
   ...
   */
  async backtestAndApplyThresholds({
    minutes = 30,
    grid = { PRICE_CHANGE_THRESHOLD: [1, 2, 3], VOLUME_MULTIPLIER: [1.5, 2, 2.5] },
  }: {
    minutes?: number;
    grid?: BacktestGrid;
    ...
    const seedTokens: string[] = JSON.parse(await fs.readFile(seedTokenFile, 'utf8'));
    ...
    const ohlcvMap: { [mint: string]: OHLCVEvent[] } = {};
    ...
    try {
      const { updateLiveParams, emitParameterUpdateEvent, incrementParameterUpdateMetric } =
        await import('../utils/selfTuning.js');
      if (best) updateLiveParams(best.params);
      if (best) emitParameterUpdateEvent(best.params);
   ...
   */
  async *streamPatternSignals({ minutes = 10 }: { minutes?: number } = {}) {
...
while (true) {
  ...
  async runPilot({ minutes = 10, maxTrades = 3 }: { minutes?: number; maxTrades?: number }) {
