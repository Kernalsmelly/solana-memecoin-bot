...
import { DataBroker } from '../integrations/data-hub/DataBroker.js';
import { RpcRotator } from '../integrations/data-hub/RpcRotator.js';
import { BirdeyeTokenData, TokenAnalyzer, AnalyzedToken } from '../analysis/tokenAnalyzer.js';
import logger from '../utils/logger.js';
import { MemoryManager } from '../utils/memoryManager.js';
import { BirdeyeAPI } from '../api/birdeyeAPI.js';
import { fetchHeliusTokenMetadata } from '../api/heliusAPI.js';
import { mockTokenDiscovery, MockToken } from '../utils/mockTokenDiscovery.js';
...
export class TokenDiscovery extends EventEmitter {
  ...
  private tokenCache: LRUCache<BirdeyeTokenData> = new LRUCache({
    maxSize: 1000,
    ttl: 60 * 60 * 1000,
  }); // 1 hour TTL
    ...
    if (testTargetToken) {
      setTimeout(() => {
        if (this.tokensDiscovered.size === 0) {
          logger.warn(
            `[TokenDiscovery] No real tokens found after ${fallbackSecs}s, emitting TEST_TARGET_TOKEN`,
          );
  ...
  public async start(): Promise<boolean> {
    ...
    raydiumWS.on('message', (data: any) => {
      try {
        const msg = JSON.parse(data.toString());
        const logs = msg?.params?.result?.value?.logs || [];
        // Look for pool-init keywords
        if (logs.some((l: string) => l.includes('InitializePool') || l.includes('AddLiquidity'))) {
