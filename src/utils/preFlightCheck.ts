...
// import { manageFunds } from './/fundManager'; // Temporarily commented out
import { RiskManager } from '../live/riskManager.js';
// import { BirdeyeAPI } from '././api/birdeyeAPI'; // Use named import
import * as fs from 'fs';
import * as os from 'os';

// dotenv.config(); // Remove this - should be loaded by index.ts before calling
  ...
  try {
    ...
    if (!configResult.isValid) {
      ...
      if (configResult.missingRequired.length > 0) {
        result.criticalIssues.push(
          `Missing required environment variables: ${configResult.missingRequired.join(', ')}`,
        );
      }
    }
    ...
    if (configResult.missingRecommended.length > 0) {
      result.warnings.push(
        `Missing recommended environment variables: ${configResult.missingRecommended.join(', ')}`,
      );
    }
    ...
    if (totalMem < 4 * 1024 * 1024 * 1024) {
      // 4GB
      result.warnings.push(
        `Low system memory: ${result.metrics.systemMemory.total}GB. Recommended: 4GB+`,
      );
    }
    ...
    if (result.criticalIssues.length === 0 && result.warnings.length <= 2) {
      result.recommendations.push(
        'System looks good. Consider starting with 5-10% of your planned capital.',
      );
    } else if (result.criticalIssues.length === 0) {
      result.recommendations.push(
        'Address warnings before live trading or start with minimal capital (1-5%).',
      );
    } else {
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      console.log(
        ` - Memory: ${result.metrics.systemMemory.free}GB free / ${result.metrics.systemMemory.total}GB total (${result.metrics.systemMemory.percentFree}% free)`,
      );
