...
function main() {
  ...
  const pools: Pool[] = parse(poolCsv, { columns: true, skip_empty_lines: true }).map(
    (row: any) => ({
      ...row,
  ...
  const trades: Trade[] = parse(tradeCsv, { columns: true, skip_empty_lines: true });

  // Build a set of traded pool addresses or token mints
  const tradedPoolAddresses = new Set<string>();
  const tradedTokens = new Set<string>();
  for (const t of trades) {
    if (t.pairAddress) tradedPoolAddresses.add(t.pairAddress);
    if (t.token) tradedTokens.add(t.token);
  }
  ...
  const avgLiquidity = pools.reduce((a: number, b: Pool) => a + b.liquidityUsd, 0) / (total || 1);
  const avgVolume = pools.reduce((a: number, b: Pool) => a + b.volume24hUsd, 0) / (total || 1);

  // Split into traded/untraded
  const traded = pools.filter((p: Pool) => p.wasTraded);
  const untraded = pools.filter((p: Pool) => !p.wasTraded);
  const avgTradedLiquidity =
    traded.reduce((a: number, b: Pool) => a + b.liquidityUsd, 0) / (traded.length || 1);
  const avgUntradedLiquidity =
    untraded.reduce((a: number, b: Pool) => a + b.liquidityUsd, 0) / (untraded.length || 1);
  const avgTradedVolume =
    traded.reduce((a: number, b: Pool) => a + b.volume24hUsd, 0) / (traded.length || 1);
  const avgUntradedVolume =
    untraded.reduce((a: number, b: Pool) => a + b.volume24hUsd, 0) / (untraded.length || 1);

  // Top missed (untraded) high-liquidity/volume pools
  const topMissed = untraded
    .filter((p: Pool) => p.liquidityUsd > 50000 && p.volume24hUsd > 10000)
    .sort((a: Pool, b: Pool) => b.liquidityUsd - a.liquidityUsd)
    .slice(0, 5);
  ...
  const tradedPoolKeys: string[] = Object.keys(poolTradeStats);
  const avgPL = tradedPoolKeys.length
    ? tradedPoolKeys.reduce(
        (a: number, k: string) => a + (Number(poolTradeStats[k]?.pnlSum) || 0),
        0,
      ) / (tradedPoolKeys.length || 1)
    : 0;
  const winRate = tradedPoolKeys.length
    ? tradedPoolKeys.reduce(
        (a: number, k: string) =>
          a +
          (Number(poolTradeStats[k]?.winCount) / (Number(poolTradeStats[k]?.tradeCount) || 1) || 0),
        0,
      ) / (tradedPoolKeys.length || 1)
    : 0;

  // --- Parameter Tuning Suggestions ---
  function median(arr: any[]): number {
    // Filter out undefined, null, and NaN just in case
    const filtered = arr.filter((n): n is number => typeof n === 'number' && !isNaN(n));
  ...
  const missedVLratio = untraded
    ...
    .filter((n): n is number => typeof n === 'number' && !isNaN(n));
  const safeMissedLiquidity = missedLiquidity.filter(
    (n): n is number => typeof n === 'number' && !isNaN(n),
  );
  const safeMissedVolume = missedVolume.filter(
    (n): n is number => typeof n === 'number' && !isNaN(n),
  );
  const safeMissedVLratio = missedVLratio.filter(
    (n): n is number => typeof n === 'number' && !isNaN(n),
  );
  const suggestedMinLiquidity =
    safeMissedLiquidity.length > 0 ? Math.min(...safeMissedLiquidity, 100000) : 100000;
  const suggestedMinVolume =
    safeMissedVolume.length > 0 ? Math.min(...safeMissedVolume, 10000) : 10000;
  const suggestedMinVLratio =
    safeMissedVLratio.length > 0 ? Math.min(...safeMissedVLratio, 0.05) : 0.05;
  ...
  console.log(
    'Avg traded liquidity:',
    avgTradedLiquidity.toFixed(2),
    '| Avg untraded liquidity:',
    avgUntradedLiquidity.toFixed(2),
  );
  console.log(
    'Avg traded 24h vol:',
    avgTradedVolume.toFixed(2),
    '| Avg untraded 24h vol:',
    avgUntradedVolume.toFixed(2),
  );
  console.log('Top 5 high-liquidity/volume missed pools:');
  topMissed.forEach((p, i) => {
    console.log(
      `${i + 1}. ${p.tokenSymbol || p.baseMint} / ${p.quoteMint} | Liquidity: $${p.liquidityUsd} | 24h Vol: $${p.volume24hUsd}`,
    );
  });
  console.log('\n--- Profitability Analysis for Traded Pools ---');
  ...
  console.log('\n--- Parameter Tuning Suggestions ---');
  console.log(
    'Current min liquidity:',
    currentMinLiquidity,
    '| Suggested (to capture missed):',
    suggestedMinLiquidity,
  );
  console.log(
    'Current min 24h volume:',
    currentMinVolume,
    '| Suggested (to capture missed):',
    suggestedMinVolume,
  );
  console.log(
    'Current min volume/liquidity ratio:',
    currentMinVLratio,
    '| Suggested:',
    suggestedMinVLratio.toFixed(4),
  );
  ...
  if (leaderboard.length > 0) {
    ...
    fs.writeFileSync(
      path.resolve(__dirname, '../../autoConfig.json'),
    ...
    const leaderboardCsv = [
      ...
      ...leaderboard.map(
        (r) =>
          `${r.minLiquidity},${r.minVolume},${r.minVLratio},${r.detected},${r.tradedPools},${r.totalPL},${r.avgPL},${r.winRate}`,
      ),
    ].join('\n');
    fs.writeFileSync(
      path.resolve(__dirname, '../../data/parameter_leaderboard.csv'),
      leaderboardCsv,
    );
    console.log(
    ...
    const missedSimResults: any[] = [];
    let totalMissedProfit15m = 0,
      totalMissedProfit1h = 0,
      totalMissedProfitBest1h = 0;
    const positionSize = 50; // Simulate $50 position
    const window15m = 15 * 60 * 1000;
    const window1h = 60 * 60 * 1000;
    const missedPools = untraded.filter(
      (p: Pool) =>
        p.liquidityUsd >= best.minLiquidity &&
        p.volume24hUsd >= best.minVolume &&
        p.volume24hUsd / (p.liquidityUsd || 1) >= best.minVLratio,
    );
    for (const pool of missedPools) {
      const detectionPriceRaw = (pool as any).priceUsd || (pool as any).price || '';
      const detectionTimeRaw = (pool as any).timestamp || (pool as any).detectedAt || '';
      const detectionPrice = parseNumber(detectionPriceRaw);
      const detectionTime = parseNumber(detectionTimeRaw);
      if (
        typeof detectionPrice !== 'number' ||
      ...
      const phRows15m = getWindowedPrices(
        priceHistory,
        pool.baseMint || '',
        pool.poolAddress,
        detectionTime,
      ...
      const phRows1h = getWindowedPrices(
        priceHistory,
        pool.baseMint || '',
        pool.poolAddress,
        detectionTime,
      ...
      const exit15m =
        lastRow15m && lastRow15m.price !== undefined ? lastRow15m.price : detectionPrice;
      ...
      const best1h =
        phRows1h.length > 0
          ? Math.max(
              ...phRows1h
                .map((r: { price: number | undefined }) => r.price)
                .filter((p: number | undefined): p is number => typeof p === 'number' && !isNaN(p)),
            )
          : detectionPrice;
    ...
    if (missedSimResults.length > 0) {
      ...
      const totalMissedProfitBest1h = missedSimResults.reduce(
        (sum, r) => sum + (r.pnlBest1h ?? 0),
        0,
      );
      console.log('\n--- Missed Opportunity Simulation (Top 10 by Best 1hr Sim P/L) ---');
      missedSimResults.slice(0, 10).forEach((r, i) => {
        console.log(
          `${i + 1}. ${r.token} | Pool: ${r.poolAddress} | Entry: $${r.detectionPrice.toFixed(6)} | Exit15m: $${r.exit15m.toFixed(6)} | P/L15m: $${r.pnl15m.toFixed(2)} | Exit1h: $${r.exit1h.toFixed(6)} | P/L1h: $${r.pnl1h.toFixed(2)} | Best1h: $${r.best1h.toFixed(6)} | BestP/L: $${r.pnlBest1h.toFixed(2)} | L: $${r.liquidityUsd} | V: $${r.volume24hUsd}`,
        );
      });
      ...
      console.log(
        `Total simulated missed profit (Best 1h): $${totalMissedProfitBest1h.toFixed(2)}`,
      );
      ...
      const missedCsv = [
        ...
        ...missedSimResults.map(
          (r) =>
            `${r.token},${r.poolAddress},${r.detectionPrice},${r.detectionTime},${r.exit15m},${r.pnl15m},${r.exit1h},${r.pnl1h},${r.best1h},${r.pnlBest1h},${r.liquidityUsd},${r.volume24hUsd}`,
        ),
      ].join('\n');
...
function parameterSweep(pools: Pool[], trades: Trade[]) {
  ...
  for (const minLiquidity of liquidityRange) {
    for (const minVolume of volumeRange) {
      for (const minVLratio of vlRatioRange) {
        const detected = pools.filter(
          (p: Pool) =>
            p.liquidityUsd >= minLiquidity &&
            p.volume24hUsd >= minVolume &&
            p.volume24hUsd / (p.liquidityUsd || 1) >= minVLratio,
        );
        const detectedKeys = new Set<string>(
          detected.map((p: Pool) => p.poolAddress || p.baseMint || ''),
        );
        let totalPL = 0,
          totalTrades = 0,
          totalWins = 0,
