...
import { loadPriceHistory, getWindowedPrices } from './priceHistoryUtils';
...
function main() {
    ...
    const pools: Pool[] = parse(poolCsv, { columns: true, skip_empty_lines: true }).map((row: any) => ({
        ...row,
    ...
    const avgTradedLiquidity = traded.reduce((a: number, b: Pool) => a + b.liquidityUsd, 0) / (traded.length || 1);
    const avgUntradedLiquidity = untraded.reduce((a: number, b: Pool) => a + b.liquidityUsd, 0) / (untraded.length || 1);
    const avgTradedVolume = traded.reduce((a: number, b: Pool) => a + b.volume24hUsd, 0) / (traded.length || 1);
    const avgUntradedVolume = untraded.reduce((a: number, b: Pool) => a + b.volume24hUsd, 0) / (untraded.length || 1);
    ...
    for (const t of trades) {
        ...
        if (pnl > 0) poolTradeStats[key].winCount++;
        poolTradeStats[key].tradeCount++;
    }
    const tradedPoolKeys: string[] = Object.keys(poolTradeStats);
    const avgPL = tradedPoolKeys.length
        ? tradedPoolKeys.reduce((a: number, k: string) =>
            a + (Number(poolTradeStats[k]?.pnlSum) || 0), 0) / (tradedPoolKeys.length || 1)
        : 0;
    const winRate = tradedPoolKeys.length
        ? tradedPoolKeys.reduce((a: number, k: string) =>
            a + ((Number(poolTradeStats[k]?.winCount) / (Number(poolTradeStats[k]?.tradeCount) || 1)) || 0), 0) / (tradedPoolKeys.length || 1)
        : 0;
    ...
    const missedLiquidity = untraded.reduce((acc: (number | undefined)[], p: Pool) => {
        if (typeof p.liquidityUsd === 'number' && isFinite(p.liquidityUsd) && !isNaN(p.liquidityUsd) && p.liquidityUsd > 0) {
            acc.push(p.liquidityUsd);
        }
        return acc;
    }, [] as (number | undefined)[]).filter((n): n is number => typeof n === 'number' && !isNaN(n));
    const missedVolume = untraded.reduce((acc: (number | undefined)[], p: Pool) => {
        if (typeof p.volume24hUsd === 'number' && isFinite(p.volume24hUsd) && !isNaN(p.volume24hUsd) && p.volume24hUsd > 0) {
            acc.push(p.volume24hUsd);
        }
        return acc;
    }, [] as (number | undefined)[]).filter((n): n is number => typeof n === 'number' && !isNaN(n));
    const missedVLratio = untraded.reduce((acc: (number | undefined)[], p: Pool) => {
        if (typeof p.liquidityUsd === 'number' && isFinite(p.liquidityUsd) && !isNaN(p.liquidityUsd) && p.liquidityUsd > 0 &&
    ...
    const safeMissedLiquidity = missedLiquidity.filter((n): n is number => typeof n === 'number' && !isNaN(n));
    const safeMissedVolume = missedVolume.filter((n): n is number => typeof n === 'number' && !isNaN(n));
    const safeMissedVLratio = missedVLratio.filter((n): n is number => typeof n === 'number' && !isNaN(n));
    ...
    if (leaderboard.length > 0) {
        ...
        const missedPools = untraded.filter((p: Pool) =>
        ...
        for (const pool of missedPools) {
            ...
            if (
                typeof detectionPrice !== 'number' || isNaN(detectionPrice) || detectionPrice <= 0 ||
            ...
            const best1h = phRows1h.length > 0
                ? Math.max(...phRows1h.map(r => r.price).filter((p): p is number => typeof p === 'number' && !isNaN(p)))
                : detectionPrice;
    ...
    for (const minLiquidity of liquidityRange) {
        for (const minVolume of volumeRange) {
            for (const minVLratio of vlRatioRange) {
                const detected = pools.filter((p: Pool) => p.liquidityUsd >= minLiquidity && p.volume24hUsd >= minVolume && (p.volume24hUsd / (p.liquidityUsd || 1)) >= minVLratio);
                const detectedKeys = new Set<string>(detected.map((p: Pool) => p.poolAddress || p.baseMint || ''));
