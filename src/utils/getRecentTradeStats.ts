 ...
 * }
 */
export async function getRecentTradeStats(windowMs: number) {
  ...
  try {
    ...
    if (trades.length > 0) {
      ...
      if (maxSym[1] > 0) mostTraded = { symbol: maxSym[0], count: maxSym[1] };
    }
    ...
    for (const t of trades) {
      ...
      if (t.pnl > 10) gt10++;
      else if (t.pnl > 0) p0to10++;
      else if (t.pnl > -10) n0to10++;
      else lt10++;
    }
