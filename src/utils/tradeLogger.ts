...
export class TradeLogger {
  ...
  public logPoolDetection(pool: Record<string, any>) {
    ...
    const keys = preferredOrder.concat(
      Object.keys(pool).filter((k) => !preferredOrder.includes(k)),
    );
  ...
  constructor(logDir = path.resolve(__dirname, '..', '..', 'data')) {
    ...
    if (!fs.existsSync(this.logFile)) {
      fs.writeFileSync(
        this.logFile,
        'timestamp,action,token,pairAddress,price,amount,pnl,reason,txid,success,strategyName,feePaidLamports,feePaidSol,slippageBps,netPnL\n',
      );
    }
  }
  ...
  public logScenario(scenarioName: string, details: Record<string, any> | string) {
    ...
    const row =
      [new Date().toISOString(), scenarioName.replace(/,/g, ';'), detailsString].join(',') + '\n';
