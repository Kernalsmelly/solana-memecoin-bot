...
export class PaperTradingEngine extends EventEmitter {
  ...
  private async closePosition(tokenAddress: string): Promise<void> {
    ...
    // Calculate PnL
    const pnl =
      (position.size * (position.currentPrice - position.entryPrice)) / position.entryPrice;
    position.pnl = pnl;
  ...
  public getRiskMetrics(): RiskMetrics {
    ...
    const profitableTrades = Array.from(this.positions.values()).filter(
      (p) => typeof p.pnl === 'number' && p.pnl > 0,
    ).length;
