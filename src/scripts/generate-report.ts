 ...
 */
function loadTradingHistory(): TradeRecord[] {
 ...
 */
function calculateDailyPerformance(allTrades: TradeRecord[], date: string): DailyPerformance {
  if (!allTrades?.length || !date) {
    return {
      date,
      startingBalance: 0,
      endingBalance: 0,
 ...
 */
function generateHtmlReport(performance: DailyPerformance): string {
      ...
      <div class="pattern-card">
        ...
        <div class="pattern-stats">
          <div class="pattern-stat">
            <div>Trades</div>
            <div><strong>${performance.patternPerformance?.[pattern]?.trades ?? 0}</strong></div>
          </div>
          <div class="pattern-stat">
            <div>Win Rate</div>
            <div><strong>${(performance.patternPerformance?.[pattern]?.winRate ?? 0).toFixed(2)}%</strong></div>
          </div>
          <div class="pattern-stat">
            <div>Avg. Profit</div>
            <div><strong>$${(performance.patternPerformance?.[pattern]?.avgProfit ?? 0).toFixed(2)}</strong></div>
          </div>
          <div class="pattern-stat">
            <div>Total Profit</div>
            <div><strong>$${(performance.patternPerformance?.[pattern]?.totalProfit ?? 0).toFixed(2)}</strong></div>
          </div>
        </div>
      </div>
    `).join('')}
 ...
 */
function generateTextReport(performance: DailyPerformance): string {
  let report = `
