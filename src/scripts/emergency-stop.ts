  ...
  try {
    ...
    if (fs.existsSync(STATE_FILE)) {
      ...
      try {
        ...
        console.error(
          `Error parsing state file: ${parseErr instanceof Error ? parseErr.message : String(parseErr)}`,
        );
    ...
    try {
      ...
      console.error(
        `Failed to send alert notification: ${alertErr instanceof Error ? alertErr.message : String(alertErr)}`,
      );
    }
    ...
    console.error(
      `\x1b[31mFailed to activate emergency stop: ${err instanceof Error ? err.message : String(err)}\x1b[0m`,
    );
    process.exit(1);
  }
}
