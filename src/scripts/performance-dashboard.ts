  ...
  try {
    ...
    if (fs.existsSync(HISTORY_FILE)) {
      try {
        ...
        console.error(
          `Error reading trade history: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
    ...
    const totalLoss =
      Math.abs(trades.filter((t) => t.pnl < 0).reduce((sum, t) => sum + t.pnl, 0)) || 1;

    const profitFactor = totalProfit / totalLoss;
    const averagePnl =
      trades.length > 0 ? trades.reduce((sum, t) => sum + t.pnl, 0) / trades.length : 0;
    ...
    console.error(
      `Error loading performance data: ${err instanceof Error ? err.message : String(err)}`,
    );
    return null;
  }
}
...
function renderDashboard(metrics: PerformanceMetrics | null): void {
  ...
  if (!metrics) {
    ...
    console.log(
      `${colors.yellow}Make sure the bot is running and the state file exists at:${colors.reset}`,
    );
  ...
  console.log(
    `${colors.blue}System Status:${colors.reset} ${statusColor}${metrics.systemEnabled ? 'ACTIVE' : 'DISABLED'}${colors.reset}`,
  );
  console.log(
    `${colors.blue}Last Updated:${colors.reset} ${new Date(metrics.lastUpdated).toLocaleString()}`,
  );
  ...
  console.log(
    `Winning Trades:    ${metrics.winningTrades} (${formatPercent(metrics.winningTrades / (metrics.totalTrades || 1))})`,
  );
  console.log(
    `Losing Trades:     ${metrics.losingTrades} (${formatPercent(metrics.losingTrades / (metrics.totalTrades || 1))})`,
  );
  ...
  console.log(
    `Dashboard updates every ${REFRESH_INTERVAL_MS / 1000} seconds. Press Ctrl+C to exit.`,
  );
  console.log(`${colors.cyan}=============================================${colors.reset}`);
}
