...
import { LiveOrderExecution, OrderExecution } from '../orderExecution';
...
export interface ManagedPosition extends Omit<Position, 'size' | 'quantity'> { // Omit potentially conflicting fields
...
export interface ExitManagerConfig {
  ...
  profitExits: {
    takeProfit: number;
    megaProfitExit?: { // Optional mega profit rule
      threshold: number;
  ...
  lossExits: {
    stopLoss: number;
    timeBasedStopAdjustment?: { // Optional time-based stop adjustment
      afterMinutes: number;
...
function deepMerge<T extends object, S extends object>(target: T, source: S): T & S {
  let output = Object.assign({}, target) as T & S;
...
export class ExitManager extends EventEmitter {
  ...
  constructor(
    ...
    birdeyeApi: BirdeyeAPI, // Add birdeyeApi parameter
   ...
   */
  public addPosition(position: ManagedPosition): void { // Use ManagedPosition
   ...
   */
  public getPositions(): ManagedPosition[] { // Return ManagedPosition array
   ...
   */
  public updatePositionExitRules(positionId: string, updates: Partial<ManagedPosition>): ManagedPosition | null { // Use ManagedPosition
   ...
   * @returns Exit reason string if triggered, null otherwise
   */
  private checkTrailingStop(position: ManagedPosition): string | null { // Use ManagedPosition
    ...
    if (position.currentPrice > highestPrice) {
      ...
      updated = true;
      logger.debug('Trailing stop adjusted upwards', { id: position.id, newStop: newStopPrice.toFixed(6) });
    }
   ...
   * @returns Exit reason string if triggered, null otherwise
   */
  private checkTimeBasedRules(position: ManagedPosition): string | null { // Use ManagedPosition
    // Use specific rules if defined for the pattern
    const patternRules = this.config.patternSpecificRules[position.pattern] || []; // Use pattern from ManagedPosition
   ...
   * @returns Exit reason string if triggered, null otherwise
   */
  private checkVolatilityRules(position: ManagedPosition): string | null { // Use ManagedPosition
   ...
   */
  private applyExitRules(position: ManagedPosition): ManagedPosition { // Use ManagedPosition
    let updatedPosition: ManagedPosition = { ...position }; // Ensure type
    ...
    const trailActivationPercent = patternTrailingRule ? (patternTrailingRule as any).activation || this.config.trailingStops.activationThreshold : this.config.trailingStops.activationThreshold;
    ...
    if (this.config.trailingStops.enabled && 
      ...
      updatedPosition.trailingStop = { // Set trailingStop on ManagedPosition
   ...
   */
  private async executeExit(position: ManagedPosition, reason: string): Promise<boolean> { // Use ManagedPosition
    ...
    try {
      ...
      if (result.success) {
        ...
        let actualSolReceived: bigint | undefined = result.inputAmount; // SOL received from sell
        let initialSolCost: bigint | undefined = position.initialSolCostLamports;
