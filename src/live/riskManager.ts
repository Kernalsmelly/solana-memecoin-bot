...
export class RiskManager extends EventEmitter {
   ...
   */
  public getDynamicPositionSizeSol(
    tokenSymbol: string,
    balance: number,
    riskPct = 0.01,
    maxExposureSol = 1,
    ...
    const variance =
      windowPrices.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / (windowPrices.length - 1);
    ...
    logger.info(
      `[RiskManager] getDynamicPositionSizeSol: sizeSOL=${size.toFixed(4)} (maxExposure=${maxExposureSol}, balance=${balance}, riskPct=${riskPct}, sigma=${sigma.toFixed(6)})`,
    );
    return size;
  }
   ...
   */
  public getPositionSizeUSD(
    tokenSymbol: string,
    currentPrice: number,
    balance: number,
    liquidityUSD: number,
  ): number {
    const MAX_EXPOSURE_USD = Number(
      process.env.MAX_EXPOSURE_USD || this.config.maxPositionValueUsd || 20,
    );
    const maxLiquidityPercent = Number(
      process.env.MAX_LIQUIDITY_PCT || this.config.maxLiquidityPercent || 0.05,
    );
    // Rolling 30min volatility (σ)
    const now = Date.now();
    const windowMs = 30 * 60 * 1000;
    const history = (this.priceHistory.get(tokenSymbol) || []).filter(
    ...
    logger.info(
      `[RiskManager] sizeUSD=${sizeUSD.toFixed(2)} (exposure=${sizeByExposure}, balance=${sizeByBalance}, liquidity=${sizeByLiquidity}, σ=${sigma.toFixed(4)})`,
    );
    return sizeUSD;
  }
  ...
  public getAccountBalance(): number {
    logger.debug(
      '[RiskManager] getAccountBalance() called, returning currentBalance: $' +
    ...
    if (this.dailyStartBalance > 0) {
      if (dailyLoss >= this.config.emergencyStopThreshold!) {
        this.triggerEmergencyStop(
          `Daily loss ${dailyLoss.toFixed(2)}% exceeds emergency threshold ${this.config.emergencyStopThreshold}%`,
        );
      }
    }
  }
  ...
  public completeTradeExecution(id: string, success: boolean, errorMessage?: string) {
    ...
    if (isNaN(timestamp)) {
      logger.error(
        `Invalid trade execution ID format: ${id} - Timestamp part is not a number: ${timestampStr}`,
      );
      return;
    }
    ...
    const execution = this.tradeExecutions.find(
      (e) => e.startTime === timestamp && e.tokenSymbol === tokenSymbol,
    );
    if (execution) {
      ...
      if (execution.executionTime > this.config.maxExecutionTime!) {
        logger.warn(
          `Trade execution time ${execution.executionTime}ms exceeds max ${this.config.maxExecutionTime}ms`,
        );
      }

      this.tradeExecutions = this.tradeExecutions.filter(
        (e) => e.startTime > Date.now() - 24 * 60 * 60 * 1000,
      );
    }
  }
